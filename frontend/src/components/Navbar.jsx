...
export default function Navbar({ activeTab, setActiveTab }) {
  ...
  useEffect(() => {
    const checkStatus = async () => {
      try {
        const result = await apiService.APIStatus();
        if (result.status == 'online') {
          setApiStatus({ loading: false, status: 'online', model: result.message });
        } else {
          throw ("Status is offline")
        }
      } catch (error) {
        setApiStatus({ loading: false, status: 'offline', model: 'None' });
      }
    };
    checkStatus();
  }, []);
